...
Finally, we could use full rate expression obtaind directly from the equilibrium exchange constant. Again we seek an expression of the form $\text{rate} = dS_{\ce{Li}}/dt = k_r (K^{\prime} C_{\ce{Li+}} - S_{\ce{Li}})$. From the equilibrium constant we obtain:
...
![Comparison of three rate laws at smal ldeviation from equilibtium on the same isotherm. $\text{log}_{10}(K_{\text{ex}})$ = 0.2, $CEC$ = 0.05 mol/kgw, $k_r$ = 0.1 m/s, $C^0_{\ce{Na+}}$ = 0.1 mol/kgw, $C^0_{\ce{Cl-}}$ = $C_{\ce{Cl-}}$ = 0.2 mol/kgw, $C_{\ce{Na+}}$ = 0.11 mol/kgw. Lithium concentration is obtained from charge balance.](../images/Small_deviation_constant_isotherm.png){#fig-case1}

@fig-case2 compares the three rates at large equilibrium deviation on the same isotherm. The performance of the small deviation rate law is poor, it under estimates desorption because it esentially decays to a notably different equilibrium state. The other rate laws perform well although are distinct.
